...
<gridLayout rows="auto,*,auto" borderRadius={4} backgroundColor={$widgetBackgroundColor} margin={2} elevation={2}>
    ...
    {/if}
    <collectionview bind:this={collectionView} row={1} items={settings} height={Math.min(300, settings.length * 80)} itemTemplateSelector={(item) => (item.type === 'switch' ? item.type : 'default')}>
        <Template let:item>
            <SettingsSlider
                icon={item.icon}
                title={item.title}
