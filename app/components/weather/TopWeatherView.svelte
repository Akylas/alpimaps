    ...
    function updateLineChart(item: Item) {
        ...
        if (chart) {
            ...
            if (!chartInitialized) {
                ...
                const limitColor = new Color(textColor).setAlpha(0.5).toRgbString();
...
<gridLayout rows="auto,*" {height} columns="*,auto">
    ...
    <canvaslabel colSpan="2" color={$textColor}>
        <cspan
            ...
            text={convertTime(item.time, 'dddd')}
        />
        ...
        {#if item.temperature !== undefined}
            <cgroup id="test" paddingLeft="10" fontSize="12" verticalAlignment="top">
                ...
                <cspan
                    ...
                    text={item.temperature !== item.apparentTemperature
        ...
        {#if (item.precipProbability === -1 || item.precipIntensity >= 0.1) && item.precipProbability > 0.1}
            <cgroup
                color={rainColor}
                paddingLeft={item.cloudCover > 0 ? 180 : 120}
                paddingTop="40"
                fontSize="14"
                verticalAlignment="top"
                width="60"
        ...
        <cspan
            ...
            text="{l('last_updated')}: {formatLastUpdate(item.lastUpdate)}"
            paddingBottom="10"
        />
    </canvaslabel>
