...
#boundaries {
  ...
  The SQL has `ORDER BY kind_detail`, so the boundary with the lowest
  kind_detail is rendered on top, and therefore the only visible boundary.
  */
    // opacity: 0.4;
    // comp-op: darken;
    // opacity: 0.5;
    [kind_detail="5"][zoom >=11] {
        // background/line-join: bevel;
        // background/line-color: white;
        // background/line-width: 2;
        // background/line-simplify: @admin-simplify;
    ...
    [kind_detail="3"],
    [kind_detail="4"],
    [zoom>=12][kind_detail="5"],
    [zoom>=13][kind_detail="6"],
    [zoom>=14][kind_detail="7"],
    [zoom>=15][kind_detail="8"],
    [zoom>=16] {
        text-name: [name];
...
#landuse[kind='aboriginal_lands'],
#landuse[kind='protected_area'] {
    text-name: [name];
    text-face-name: @book-fonts;
    text-fill: green;
    [kind='aboriginal_lands'],
    [kind='protected_area'][protect_class='24'] {
