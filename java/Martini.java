...
public class Martini {
    ...
    @NonNull
    public final Martini.Tile createTile(@NonNull double[] terrain) throws Throwable {
    ...
    static final class Tile {
        ...
        @NonNull
        public final Martini getMartini() {
            return this.martini;
        }
        ...
        public final void update() {
            ...
            for (int i = numTriangles - 1; i >= 0; --i) {
                ...
                int cx = mx + my - ay;
                int cy = my + ax - mx;
                double interpolatedHeight = (this.terrain[ay * size + ax] + this.terrain[by * size + bx]) / (double) 2;
                int middleIndex = my * size + mx;
                double middleError = Math.abs(interpolatedHeight - this.terrain[middleIndex]);
        ...
        public final void setVertices(@NonNull int[] var1) {
            this.vertices = var1;
        }
        ...
        public final void setTriangles(@NonNull int[] var1) {
            this.triangles = var1;
        }
        ...
        public final void processTriangle(int ax, int ay, int bx, int by, int cx, int cy, int maxError) {
            int mx = ax + bx >> 1;
            int my = ay + by >> 1;
            if (Math.abs(ax - cx) + Math.abs(ay - cy) > 1 && this.errors[my * this.size + mx] > (double) maxError) {
        ...
        public final Pair<int[], int[]> getMesh(int maxError) {
            int size = this.martini.getGridSize();
            int[] indices = this.martini.getIndices();
            int numVertices = 0;
            int numTriangles = 0;
            int max = size - 1;
            ...
            this.vertices = new int[numVertices * 2];
        ...
        public Tile(@NonNull double[] terrain, @NonNull Martini martini) throws Throwable {
            super();
            this.indices = new int[0];
            this.vertices = new int[0];
            this.triangles = new int[0];
            int size = martini.getGridSize();
