...
2. If you are on `master` (this repo's default branch — it is not `main`), you must branch — never edit directly on `master`.
3. Ensure `master` is current first: `git pull origin master` (rebase onto `upstream/master` if working from a fork).
