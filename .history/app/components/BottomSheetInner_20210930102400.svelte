<script lang="ts">
    ...
    let activityIndicator: NativeViewElementNode<View>;
    ...
    $: {
        ...
        if (activityIndicator && loadedListeners.length > 0) {
...
<gridlayout {...$$restProps} width="100%" rows={`${INFOVIEW_HEIGHT},50,${profileHeight},auto`} columns="*,auto" backgroundColor={$widgetBackgroundColor} on:tap={() => {}}>
    {#if loaded}
        <ErrorBoundary>
            ...
            <mdactivityindicator visibility={updatingItem ? 'visible' : 'collapsed'} horizontalAligment="right" busy={true} width={20} height={20}  bind:this={activityIndicator}/>
